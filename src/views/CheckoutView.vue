    ...
    <div class="grid sm:px-10 lg:grid-cols-2 lg:px-20 xl:px-32">
      <div class="px-4 pt-8">
        ...
        >
          ...
          <div v-for="data in getCarts" :key="data.cart_id" class="flex flex-col rounded-lg bg-white sm:flex-row">
            <img
            ...
            <div class="flex w-full flex-col px-4 py-4">
              <span class="font-semibold"
                >{{ data.name }}</span
              >
              <span class="float-right text-gray-400">{{ data.qty }}</span>
        ...
        <div class="">
          ...
          <div class="flex flex-col sm:flex-row">
            <div class="relative flex-shrink-0 sm:w-7/12">
              ...
              > {{ getAddres[0].address }}, {{ getAddres[0].city }}, {{ getAddres[0].state }} </span>
              <div
                class="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3"
              >
                <img
          ...
          <div class="mt-6 border-t border-b py-2">
            <div class="flex items-center justify-between">
              ...
              <p class="font-semibold text-gray-900">{{ formatCurrency(calculateTotal) }}</p>
            </div>
            <div class="flex items-center justify-between">
              <p class="text-sm font-medium text-gray-900">Shipping</p>
              <p class="font-semibold text-gray-900">{{formatCurrency(shippingCost)}}</p>
            </div>
          </div>
          <div class="mt-6 flex items-center justify-between">
            <p class="text-sm font-medium text-gray-900">Total</p>
            <p class="text-2xl font-semibold text-gray-900">{{ formatCurrency(calculateTotal) }}</p>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['getCarts', 'getAddres','getCheckout']),

    calculateTotal() {
    return (
      this.getCarts.reduce((total, cartItem) => {
        return total + cartItem.regular_price * cartItem.qty;
      }, 0) +
      this.shippingCost // Tambahkan biaya pengiriman ke total
    );
  },
  ...
  methods: {
    ...
    ...mapActions('cart', ['fetchCarts', 'fetchAddres', 'checkoutCart', 'getAddres']),
    ...
    async performCheckout() {
            ...
            const cartItemIds = this.getCarts.map(product => product.cart_id);
            // Get user address
            const userAddressResponse = await this.$store.dispatch('cart/fetchAddres');
